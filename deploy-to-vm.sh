#!/bin/bash
# Run this script ON THE VM (via SSH) to update the backend files
# Usage: paste this entire script into your VM terminal

echo "=== Updating NearShop API backend ==="

# 1. Update products/router.py - add suggestions endpoint + optional radius_km
cat > /opt/nearshop/app/products/router.py << 'ROUTER_EOF'
...
async def get_similar_products_endpoint(
    ...
    product = await get_product(db, product_id)
    products, total = await search_products(
        db, query=None, category=product.category, page=page, per_page=per_page,
    )
    items = [_product_to_response(p) for p in products if p.id != product_id]
    return ProductListResponse(items=items, total=max(total - 1, 0), page=page, per_page=per_page)
ROUTER_EOF

echo "  [1/2] router.py updated"
...
if systemctl is-active --quiet nearshop-api; then
    echo "  [2/2] Service restarted successfully"
else
    echo "  [2/2] ERROR: Service failed to start!"
...
curl -s http://localhost:8000/api/v1/products/suggestions?q=test | python3 -m json.tool 2>/dev/null || echo "Suggestions endpoint test failed"
echo ""
echo "=== Done! ==="
