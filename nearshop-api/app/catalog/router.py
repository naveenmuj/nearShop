...
async def add_catalog_products_to_shop(
    ...
    for selection in body.items:
        ...
        existing_product_id = existing_result.scalar_one_or_none()
        ...
        if existing_product_id:
            existing_product_result = await db.execute(
                select(Product).where(Product.id == existing_product_id)
            )
            existing_product = existing_product_result.scalar_one_or_none()
            if existing_product:
                existing_product.price = price_value
                existing_product.compare_price = compare_price
                existing_product.description = selection.local_description or template.description or template.short_description
                ...
                existing_product.tags = [tag for tag in [template.brand, template.category, template.subcategory] if tag]
                existing_product.images = template.image_urls or ([template.thumbnail_url] if template.thumbnail_url else [])
                created.append(
                    CatalogAddResultItem(
                        catalog_id=template.id,
                        ...
                        name=template.name,
                        status="updated",
                        message="Price and stock overrides updated",
                    )
                )
                updated_count += 1
                continue
        ...
        created.append(
            CatalogAddResultItem(
                ...
                name=template.name,
                status="created",
                message="Added to your shop catalog as hidden",
            )
        )
        created_count += 1
